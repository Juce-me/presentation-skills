...
**Resulting thoughts must dominate.** When a slide builds toward a lesson, standard, decision, pattern, principle, or "so what," that resulting thought is not supporting copy. It must either become its own slide, or become the dominant final state of the current slide: a large hero statement or top-layer conclusion overlay above the evidence. Do not park it as a small footer, italic caption, quiet subtitle, lower bridge line, or muted note under larger objects.

Default treatment: reveal the evidence first, then reveal the resulting thought last as the biggest element after the title, with the evidence still recognizable but visually subordinate. If the resulting thought is another idea rather than the synthesis of the evidence, split it.
